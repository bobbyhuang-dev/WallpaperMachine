...
const paths = {
  ...
  chevronDown: '<path d="m6 9 6 6 6-6"/>', chevronRight: '<path d="m9 5 7 7-7 7"/>',
  chevronLeft: '<path d="m15 5-7 7 7 7"/>', check: '<path d="m5 12 4 4L19 6"/>',
  refresh: '<path d="M20 7v5h-5M4 17v-5h5"/><path d="M6.1 6.1A8 8 0 0 1 20 12M4 12a8 8 0 0 0 13.9 5.9"/>',
  folder: '<path d="M3 7V5h6l2 2h10v12H3Z"/>', external: '<path d="M14 3h7v7M21 3l-11 11M10 5H4v15h15v-6"/>',
  ...
  download: '<path d="M12 3v12m-5-5 5 5 5-5M4 16v5h16v-5"/>', close: '<path d="m6 6 12 12M6 18 18 6"/>',
  star: '<path d="m12 3 2.8 5.7 6.2.9-4.5 4.4 1.1 6.2-5.6-3-5.6 3 1.1-6.2L3 9.6l6.2-.9Z"/>',
  image: '<rect x="3" y="3" width="18" height="18" rx="1"/><circle cx="8" cy="8" r="1.5"/><path d="m3 17 6-6 4 4 3-3 5 5"/>',
  trash: '<path d="M3 6h18M9 6V3h6v3M5 6l1 15h12l1-15M10 10v7M14 10v7"/>',
  filter: '<path d="M3 5h18M6 12h12M9 19h6"/>', plus: '<path d="M12 4v16M4 12h16"/>',
  shield: '<path d="m12 3 8 3v6c0 5-8 9-8 9s-8-4-8-9V6Z"/><path d="m8 12 3 3 5-6"/>'
};
function icon(name, size = 16) { return `<svg width="${Number(size) || 16}" height="${Number(size) || 16}" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.65" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true">${paths[name] || paths.info}</svg>`; }
...
function button(label, action, args = {}, options = {}) {
  ...
  return `<button type="button" data-action="${action}" ${attributes} class="${options.className || ''}"${disabled(options.disabled || busy(action, args))}${options.title ? ` title="${escapeHTML(options.title)}" aria-label="${escapeHTML(options.title)}"` : ''}>${options.icon ? icon(options.icon) : ''}${label ? `<span class="button-label">${escapeHTML(label)}</span>` : ''}</button>`;
}
...
function renderInspector(discover) {
  ...
  const target = state.displays.find(display => display.id === state.targetDisplayID);
  const canActivate = isInstalled && !['Web', 'Application', 'Unknown'].includes(item.kind) && target?.enabled && target.mode !== 'mirror';
...
function queueState() {
  ...
  const summary = running ? percent === null ? 'Downloading' : `Downloading · ${percent}%` : requests.length ? `${requests.length} download${requests.length === 1 ? '' : 's'} need setup` : queued.length ? `${queued.length} waiting to download` : downloads.length ? `${downloads.length} download${downloads.length === 1 ? '' : 's'}` : 'No downloads';
