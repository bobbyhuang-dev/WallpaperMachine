...
function draw(view) {
  ...
  const performance = `<h3>${e(t('Video backend'))}</h3>`
    ...
    + row('scene-renderer', t('Scene renderer'), select('sceneRenderer', t('Scene renderer'), draft('sceneRenderer', settings.sceneRenderer), localizedOptions(sceneRenderers), 'data-setting="sceneRenderer"', busy || unavailable), t('Native Metal draws only scenes it can draw in full: image layers, sprite-sheet animation, standard two-dimensional sprite particles, ordinary effect chains and post-processing, same-frame layer links, and BGRA or 8-bit NV12 video textures. Anything else — rope and trail particles, puppets, 3D, dynamic lighting, history-feedback effects, HDR video and the rest — falls back to Compatibility as a whole scene, and this applies to desktop wallpapers only, the lock screen staying on Compatibility.'))
  ...
  const displays = (state.displays || []).map(display => {
    ...
    return `<div class="settings-display" data-key="display-${e(id)}"><h3>${e(display.title)}${primary ? `<span class="settings-note">${e(t('Primary display'))}</span>` : ''}</h3>`
      ...
      + disclosure(key('advanced'), t('Playback & scaling'), (!mirror && !display.wallpaperID ? `<div class="settings-note">${e(t('Choose a wallpaper to adjust playback.'))}</div>` : '') + row(key('scaling-row'), t('Scaling'), select(key('scalingMode'), name(t('Scaling')), draft(key('scalingMode'), playback.scalingMode), localizedOptions([['none', 'None'], ['stretch', 'Stretch'], ['match', 'Match'], ['fill', 'Fill']]), data('scalingMode'), playbackOff))
