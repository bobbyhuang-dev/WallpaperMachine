# Verification log archive — 2026-09 and earlier
...
level. The original text of every entry, including any removed before this
archive existed, stays in Git history
(`git log -p -- docs/testing/verification-log.md`).

These are historical results about the trees they were taken on. They are not
evidence about the current tree and must never be cited as such. Durable facts
worth keeping were promoted into the reference documents that own them:
renderer behaviour and known-failing tests into
...
- Headless Chromium: 84 English/Chinese, light/dark, seven-category layout cases at 760/960/1440px passed; 16 long-content cases passed. Exercised battery controls, typed numeric values, disclosure state, language switching, and renderer-unavailable controls with synthetic snapshots.
- Final current-source isolated Settings smoke: 42 category/locale/width cases passed; aligned navigation icons, no horizontal overflow, keyboard navigation, focus and disclosure preservation passed.
- python3 scripts/test.py --only ControlPanelShellTests initially passed: 11 passed, 0 failed, 0 skipped before the unrelated panel dependency appeared.
- python3 scripts/test.py: initial Python gate found two missing Chinese inspector keys; added those translations. Retry passed all 103 Python tests; native result 507 passed, 32 failed, 11 skipped of 550.
- Serial settings navigation rerun failed during panel startup. Current panel.js imports missing WebUI/property-label.js; headless reload independently reports that module request failing. Module is also absent from the native asset allowlist. Preserved unrelated work; full gate remains blocked.
- node --check passed for settings.js and zh-Hans.js. Impeccable detector reported no findings.
- Desktop visuals, screenshots, live renderer, audio, Steam, and OS integration not exercised. No Release build or app restart; running app unchanged.
...
- python3 scripts/build.py --swift-only --configuration Release succeeded; 25 warning lines in build log.
- Verified all 11 current WebUI source files against bundled Contents/Resources/WebUI byte-for-byte.
- Extracted built AppIcon.icns using iconutil and visually confirmed the new charcoal, gradient screen and gear icon.
- Delivered build/Build/Products/Release/WallpaperMachine.app. App not launched or quit; user must quit and reopen.
...
## 2026-09-22 — WallpaperMachine brand and centered app icon

- Generated app and tray assets plus website logos, SVG/ICO/PNG favicons and manifest with python3 scripts/brand.py --website ../WallpaperMachineWebiste.
- Corrected foreground optical centering consistently across app, touch and favicon exports.
- Headless Chromium: real panel rendered with illustrative snapshots; title-bar, About and welcome SVG marks present; visually inspected About and settled welcome layouts. Native desktop not launched.
- python3 scripts/test.py: Python checks passed; native 535 passed, 0 failed, 11 skipped of 546. 16 warning lines reported.
- Impeccable detector on changed WebUI JS/CSS returned no findings.
- Release app not rebuilt; existing running app still uses old assets. Website directory is asset-only; HTML head integration documented in appearance.md.

## 2026-09-22 — Center welcome language labels

- Language tiles with no subtitle center their label; System (Auto) stays left-aligned with its note.
- Fixture measurement: English and 简体中文 title centers match the tile center; the selected check does not cover the label. Appearance tiles unchanged.
- `python3 scripts/test.py` — exit 0; 535 passed, 0 failed, 11 skipped of 546.
- `python3 scripts/build.py --swift-only --configuration Release` — exit 0. Bundled `WebUI/welcome.css` and `welcome.js` match source.
