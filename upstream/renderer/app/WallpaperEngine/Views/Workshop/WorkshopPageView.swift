...
struct WorkshopPageView: View {
    ...
    var body: some View {
                ...
                ScrollView {
                    LazyVGrid(columns: [GridItem(.adaptive(minimum: 240, maximum: 360), spacing: 18)], spacing: 18) {
                        ForEach(workshop.items) { item in
                            WorkshopCard(item: item, installed: isInstalled(item.id)) { selectedItem = item }
                        }
        ...
        .onAppear {
            if workshop.downloader.isRunning,
               let id = workshop.downloader.currentItemID {
                selectedItem = workshop.items.first { $0.id == id }
            }
        }
    ...
    private func isInstalled(_ id: String) -> Bool {
        bridge.librarySnapshot.wallpapers.contains { $0.id == id } || workshop.downloader.downloadedID == id
    }
}
...
private struct WorkshopCard: View {
    ...
    var body: some View {
        Button(action: action) {
            VStack(alignment: .leading, spacing: 0) {
                WorkshopPreview(url: item.previewURL)
                    ...
                    .overlay(alignment: .topTrailing) {
                        if installed { Label("In Library", systemImage: "checkmark").font(.caption.weight(.medium)).padding(7).background(.regularMaterial, in: Capsule()).padding(10) }
                    }
            ...
            .accessibilityLabel("\(item.title), \(item.kind.compatibility)\(installed ? ", in library" : "")")
