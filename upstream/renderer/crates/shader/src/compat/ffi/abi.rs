...
/// Compiler identity used by the renderer's persistent program cache.
#[unsafe(no_mangle)]
pub extern "C" fn rs_shader_cache_identity() -> *const c_char {
    static IDENTITY: std::sync::OnceLock<std::ffi::CString> = std::sync::OnceLock::new();
    IDENTITY.get_or_init(|| std::ffi::CString::new(format!(
        "{}:{}",
        crate::pipeline::ShaderPipelineRevision::CURRENT.value(),
        crate::pipeline::ShaderPipeline::<(), (), ()>::compiler_options_cache_salt(),
    )).expect("compiler identity contains no NUL")).as_ptr()
}
...
pub fn ensure_linked() {
    ...
    let _ = std::hint::black_box(rs_shader_cache_identity as *const ());
